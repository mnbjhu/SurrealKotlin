   ...
   return filter { declaration ->
        declaration.annotations.any {
            it.annotationType
                .resolve()
                .starProjection()
                .isAssignableFrom<uk.gibby.dsl.annotation.Relation<*, *>>(resolver)
        }
...
fun buildRelationClasses(codeGenerator: CodeGenerator, resolver: Resolver, logger: KSPLogger, classes: Sequence<KSClassDeclaration>) {
    classes.forEach {
        val annotation = it.annotations.first { it.annotationType.resolve().starProjection().isAssignableFrom<uk.gibby.dsl.annotation.Relation<*, *>>(resolver) }
        ...
        builder.addProperty(
            PropertySpec.builder(
                ...
                uk.gibby.dsl.core.Table::class.asTypeName().parameterizedBy(
                    it.toClassName(),
